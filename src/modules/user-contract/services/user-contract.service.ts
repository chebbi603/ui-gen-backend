...
export class UserContractService {
  constructor(@InjectModel(UserContract.name) private readonly userContractModel: Model<UserContract>) {}

  async getUserContract(userId: string) {
    const doc = await this.userContractModel.findOne({ userId: new MongooseTypes.ObjectId(userId) }).sort({ updatedAt: -1 });
    return doc;
  }

  async upsertUserContract(userId: string, contractId: string | undefined, json: any, requesterId: string, requesterRole: string) {
    if (requesterId !== userId && requesterRole !== 'ADMIN') {
      throw new ForbiddenException('Cannot modify other user contract');
    }
    const { valid, errors } = validateContractJson(json);
    if (!valid) throw new BadRequestException({ message: 'Invalid contract json', errors });
