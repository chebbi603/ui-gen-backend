...
export class ContractService {
  constructor(@InjectModel(Contract.name) private readonly contractModel: Model<Contract>) {}

  async create(json: any, version: string, meta: any, createdBy: string) {
    const { valid, errors } = validateContractJson(json);
    if (!valid) {
      throw new BadRequestException({ message: 'Invalid contract json', errors });
    }
  ...
  async findById(id: string) {
    const doc = await this.contractModel.findById(new MongooseTypes.ObjectId(id));
